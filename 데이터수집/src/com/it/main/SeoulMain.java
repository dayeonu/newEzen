...

	{
...

		{
...

			{
				...
				Elements poster = doc.select("ul.article-list li.item div.thumb"); // 장소 이미지 

				Elements comment = doc.select("ul.article-list li.item div.infor-element span.text-dot-d"); // poster의 장소 설명 
...

				{
					...
					try{
						...
						System.out.println(poster.get(j).attr("style")); // 장송 이미지 주소 가져오기 /comm/getImage?srvcId=POST&parentSn=1153&fileTy=POSTTHUMB&fileNo=2&thumbTy=M) 

						System.out.println(comment.get(j).text()); // 텍스트만 뽑아오는 메서드 

						System.out.println(link.get(j).attr("href"));

						Document doc2=Jsoup.connect("https://korean.visitseoul.net"+link.get(j).attr("href")).get();

						Element address = doc2.select("div.detail-map-infor:eq(1) dl dd").get(0); // 사진속 장소의 주소 및 교통정보 
